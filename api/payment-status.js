export default async function handler(req, res) {
  ...
  if (!/^LC12\d{8}$/.test(code)) return res.status(400).json({ ok: false, error: 'Invalid order code' });
  ...
  } catch (error) { return res.status(502).json({ ok: false, error: 'Could not check payment status' }); }
}
